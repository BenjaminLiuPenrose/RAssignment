...
```{r}
pdf("airline_plot.pdf", height=5, width=5)
x <- rnorm(10); y <- rnorm(10)
plot(x, y)
dev.off()

pdf("airline_plot_2.pdf", height=30, width=30)
x <- rnorm(10); y <- rnorm(10)
plot(x, y)
dev.off()
```
...
```{r}
path.package("graphics", quiet = FALSE)
true_false <- path.package("graphics", quiet = FALSE)==path.package("graphics", quiet = FALSE)
```
