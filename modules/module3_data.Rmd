...
```{r}
cnt_na_dep = sum(is.na(air$DepDelay))
```
...
```{r}
my_str <- paste(air$UniqueCarrier[1], air$FlightNum[1], sep="-")
```
...
```{r}
my_str_2 <- paste("There are ", length(air$FlightNum) ," rows in the dataset", sep="")
```
...
```{r}
num_all_flights <- length(air$Year)
bool_WN <- air$UniqueCarrier=="WN"
bool_onek <- air$Distance<1000
bool_choice <- bool_WN & bool_onek
num_wn_flight <- length(air$Year[bool_choice])
pct <- num_wn_flight/num_all_flights
```

9) Use that vector to create a new data frame that is a subset of the original data frame.

```{r}
sub <- air[bool_choice, c("Dest", "Origin")]
```
