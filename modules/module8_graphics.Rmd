...
* `ggplot2` plays nice with `dplyr` and pipes. If you want to manipulate your data specifically for one plot but 9 save the new dataset, you can call your `dplyr` chain and pipe it directly into a `ggplot` call.
...
```{r}
data_one <- air$flighTDelay
data_one[data_one<0] <- 0
data_one[data_one]
```
